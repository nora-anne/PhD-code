...
def tns_tts_nplanet_l(M,orbits,t0,tmax,tref):
    """
    Returns transit times and transit numbers for an N planet system.
    Uses the -z axis as the line of sight.
    M is array of masses in same units (assumed to be solar masses).
...
##    plt.show()
    ...
    #set up array for saving transit times and numbers (need numbers in case a transit is missed)
    transittimes = np.array([])
    transnums = np.array([],dtype=int)
    pflags = np.array([],dtype=int)
    #do for each planet
    for i in range(N):
        transittimesi = []
        #integrate backwards to t0
        sim = sim0.copy()
        sim.dt = -sim0.dt
        p = sim.particles
        unstable=False
        while sim.t >= t0:
            ...
            if x_old*x_new<0 and p[i+1].z-p[0].z<0: 
                while t_old-t_new>1e-7:   # bisect until prec of 1e-7 reached
                    if x_old*x_new<0.:
                        t_new = sim.t
                    else:
                        t_old = sim.t
                    ...
                    x_new = p[i+1].x - p[0].x
                if p[i+1].z-p[0].z<0:
                    transittimesi.append(sim.t) #records transit time
                sim.integrate(sim.t-0.01)       # integrate to be past the transit
            if p[i+1].a<0 and unstable==False:
                print('unstable planet',i+1,'a=',p[i+1].a,'e=',p[i+1].e,'t=',sim.t)
                unstable=True
            
        #integrate forward to tmax
        sim = sim0.copy()
        p = sim.particles
        unstable=False
        while sim.t <= tmax:
        ...
        transittimesi = np.array(transittimesi)
        transittimes = np.concatenate((transittimes,np.sort(transittimesi)))
        transnums = np.concatenate((transnums,
                                    np.arange(0,len(transittimesi),step=1,dtype=int)))
        pflags = np.concatenate((pflags,np.full(len(transittimesi),i+1,dtype=int)))
##    print(sim.t)
##    for i in range(N):
##        print(p[i+1].P,p[i+1].e,p[i+1].inc,p[i+1].Omega,
##              p[i+1].omega,p[i+1].l)
    return pflags,transnums,transittimes

def tns_tts_nplanet_l_xLOS_withdur(M,R,orbits,t0,tmax,tref):
    """
    Returns transit times and transit numbers for an N planet system along with transit durations.
    Uses the +x axis as the line of sight.
    M is array of masses in same units (assumed to be earth masses).
    R is array of radii in same units (assumed to be AU).
    orbits is an array of orbital arrays Keplerian orbital elements (P,e,inc,Om,om,l) in days/rads.
    t0/tmax are start/end of integration time (days).
    """
...
##    plt.show()
    ...
    for i in range(N):
        ...
        while sim.t >= t0:
            ...
            t_new = sim.t
            y_new = p[i+1].y - p[0].y
            if y_old*y_new<0 and p[i+1].x-p[0].x>0: 
                while t_old-t_new>1e-7:   # bisect until prec of 1e-7 reached
                    if y_old*y_new<0.:
                        t_new = sim.t
                    else:
                ...
                if p[i+1].x-p[0].x>0:
                    transittimesi.append(sim.t) #records transit time
                    rho = p[i+1].a/(R[i+1]+R[0])
                    duration = (1-p[i+1].e**2)**.5/(1+p[i+1].e*np.cos(p[i+1].pomega)) *np.sqrt(
                        1-rho**2*((1-p[i+1].e**2)/(1+p[i+1].e*np.cos(p[i+1].pomega)))**2*(np.cos(p[i+1].inc))**2)
                    tdursi.append(duration)
                sim.integrate(sim.t-0.01)       # integrate to be past the transit
            if p[i+1].a<0 and unstable==False:
                print('unstable planet',i+1,'a=',p[i+1].a,'e=',p[i+1].e,'t=',sim.t)
                unstable=True
        ...
        tdursi = np.array(tdursi)
        tdurs = np.concatenate((tdurs,tdursi[np.argsort(transittimesi)]))
        transittimesi = np.array(transittimesi)
        transittimes = np.concatenate((transittimes,np.sort(transittimesi)))
        transnums = np.concatenate((transnums,
                                    np.arange(0,len(transittimesi),step=1,dtype=int)))
...
##        print(p[i+1].P,p[i+1].e,p[i+1].inc,p[i+1].Omega,
##              p[i+1].omega,p[i+1].l)
    return pflags,transnums,transittimes,tdurs
